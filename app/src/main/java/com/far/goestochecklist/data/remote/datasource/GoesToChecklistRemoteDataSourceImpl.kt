...
import com.far.goestochecklist.data.mapper.LoginMapper
...
class GoesToChecklistRemoteDataSourceImpl(
	...
	override fun signUp(name: String, username: String, password: String) = flow {
		emit(
			wrapper.wrapper { service.signUp(UserRequest(name, username, password)) }
		)
	}
	...
	override fun login(username: String, password: String) = flow {
		emit(
			LoginMapper.toDomain(
				wrapper.wrapper { service.login(LoginRequest(username, password)) }
			)
		)
	}
}